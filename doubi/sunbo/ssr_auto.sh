...
BBR_installation_status(){
	if [[ ! -e ${BBR_file} ]]; then
		...
		cd "${file}"
		if ! wget -N --no-check-certificate https://raw.githubusercontent.com/disanshijie/VPN/master/doubi/sunbo/bbr.sh; then
...
Set_config_port(){
	...
	stty erase '^H' && read -p "(默认: 9999):" ssr_port
	[[ -z "$ssr_port" ]] && ssr_port="9999"
...
Set_config_password(){
	...
	stty erase '^H' && read -p "(默认: sunjinchao123):" ssr_password
	[[ -z "${ssr_password}" ]] && ssr_password="sunjinchao123"
...
Set_config_protocol_param(){
	...
	[[ -z "$ssr_protocol_param" ]] && ssr_protocol_param="5"
...
Download_SSR(){
	cd "/usr/local/"
	wget -N --no-check-certificate "https://raw.githubusercontent.com/disanshijie/VPN/master/doubi/file/manyuser.zip"
...
Service_SSR(){
	if [[ ${release} = "centos" ]]; then
		if ! wget --no-check-certificate https://raw.githubusercontent.com/disanshijie/VPN/master/doubi/src/module/ssr_centos -O /etc/init.d/ssr; then
		...
		if ! wget --no-check-certificate https://raw.githubusercontent.com/disanshijie/VPN/master/doubi/src/module/ssr_debian -O /etc/init.d/ssr; then
			echo -e "${Error} ShadowsocksR服务 管理脚本下载失败 !" && exit 1
		fi
		chmod +x /etc/init.d/ssr
